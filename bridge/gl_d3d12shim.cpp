...
static bool IsCompressedTextureFormat(GLenum fmt)
{
  return fmt == GL_COMPRESSED_RGBA_S3TC_DXT1_EXT ||
    fmt == GL_COMPRESSED_RGBA_S3TC_DXT5_EXT;
}

static UINT QD3D12_BlockSizeForCompressedFormat(GLenum fmt)
{
  switch (fmt)
  {
  case GL_COMPRESSED_RGBA_S3TC_DXT1_EXT: return 8;   // BC1
  case GL_COMPRESSED_RGBA_S3TC_DXT5_EXT: return 16;  // BC3
  default: return 0;
  }
}

static DXGI_FORMAT MapCompressedTextureFormat(GLenum fmt)
{
  switch (fmt)
  {
  case GL_COMPRESSED_RGBA_S3TC_DXT1_EXT: return DXGI_FORMAT_BC1_UNORM;
  case GL_COMPRESSED_RGBA_S3TC_DXT5_EXT: return DXGI_FORMAT_BC3_UNORM;
  default: return DXGI_FORMAT_UNKNOWN;
  }
}

static size_t QD3D12_CompressedImageSize(GLenum fmt, int width, int height)
{
  const UINT blockBytes = QD3D12_BlockSizeForCompressedFormat(fmt);
  if (blockBytes == 0 || width <= 0 || height <= 0)
    return 0;

  const UINT blocksWide = (UINT)((width + 3) / 4);
  const UINT blocksHigh = (UINT)((height + 3) / 4);
  return (size_t)blocksWide * (size_t)blocksHigh * (size_t)blockBytes;
}
...
static const UINT QD3D12_MaxTextureUnits = 12;
...
struct TextureResource
{
  ...
  GLenum type = GL_UNSIGNED_BYTE;   // add this
...
extern "C" void  glSelectTextureSGIS(GLenum texture)
{
    switch (texture)
    {
...
extern "C" void  glMTexCoord2fSGIS(GLenum texture, GLfloat s, GLfloat t)
{
...
extern "C" void  glMultiTexCoord2fARB(GLenum texture, GLfloat s, GLfloat t)
{
    GLuint unit = 0;
    if (texture >= GL_TEXTURE0_ARB)
...
static DXGI_FORMAT MapTextureFormat(GLenum format, GLenum type = GL_UNSIGNED_BYTE)
{
  switch (format)
  {
  case GL_ALPHA:
...
{
  tex.dxgiFormat = tex.compressed
    ? MapCompressedTextureFormat(tex.compressedFormat)
    : MapTextureFormat(tex.format, tex.type);
...
static UINT QD3D12_TextureRowBytes(const TextureResource& tex)
{
  if (tex.compressed)
  {
    const UINT blockBytes = QD3D12_BlockSizeForCompressedFormat(tex.compressedFormat);
    const UINT blocksWide = (UINT)((tex.width + 3) / 4);
    return blocksWide * blockBytes;
  }

  const int bpp = BytesPerPixel(tex.format, tex.type);
  return (UINT)(tex.width * bpp);
}
...
static void UploadTexture(TextureResource& tex)
{
  ...
  if (tex.dxgiFormat == DXGI_FORMAT_UNKNOWN)
  {
    QD3D12_Fatal(
      "UploadTexture: unsupported texture format/type (gl format=0x%X type=0x%X)",
      (unsigned)tex.format,
      (unsigned)tex.type);
    return;
  }
  ...
  UploadAlloc alloc = QD3D12_AllocUpload(
    (UINT)totalBytes,
    D3D12_TEXTURE_DATA_PLACEMENT_ALIGNMENT);

  uint8_t* dstBase = reinterpret_cast<uint8_t*>(alloc.cpu);
  memset(dstBase, 0, (size_t)totalBytes);

  if (tex.compressed)
  {
    const UINT blockBytes = QD3D12_BlockSizeForCompressedFormat(tex.compressedFormat);
    const UINT blocksWide = (UINT)((tex.width + 3) / 4);
    const UINT blocksHigh = (UINT)((tex.height + 3) / 4);
    ...
    if (!tex.sysmem.empty())
    {
      ...
      {
        memcpy(
          dstBase + footprint.Offset + (size_t)y * footprint.Footprint.RowPitch,
          srcBase + (size_t)y * srcRowBytes,
          srcRowBytes);
      }
    }
  }
  else
  {
    ...
    {
      QD3D12_Fatal(
        "UploadTexture: unsupported uncompressed texture format/type (gl format=0x%X type=0x%X)",
        (unsigned)tex.format,
        (unsigned)tex.type);
      return;
    }

    const UINT srcRowBytes = (UINT)(tex.width * bpp);
  ...
  D3D12_RESOURCE_BARRIER b{};
  b.Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
  b.Transition.pResource = tex.texture.Get();
  b.Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
  b.Transition.StateBefore = D3D12_RESOURCE_STATE_COPY_DEST;
  b.Transition.StateAfter = D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE;
...
extern "C" void  glFrustum(GLdouble left, GLdouble right,
...
extern "C" void  glTexParameterf(GLenum, GLenum pname, GLfloat param)
{
...
extern "C" void glTexImage2D(GLenum, GLint level, GLint internalFormat,
  GLsizei width, GLsizei height, GLint, GLenum format, GLenum type, const GLvoid* pixels)
{
  ...
  if (level > 0)
    return; // todo: mips but dont take the games mips the game mips are probably shit.
  ...
  const GLenum requestedFormat =
    (format != 0) ? format : (GLenum)internalFormat;

  tex.format = requestedFormat;
  tex.type = type;
  tex.compressed = IsCompressedTextureFormat((GLenum)internalFormat) ||
    IsCompressedTextureFormat(requestedFormat);
  tex.compressedFormat = tex.compressed
    ? (IsCompressedTextureFormat((GLenum)internalFormat)
  ...
  tex.dxgiFormat = tex.compressed
    ? MapCompressedTextureFormat(tex.compressedFormat)
    : MapTextureFormat(tex.format, type);

  if (tex.compressed)
  {
    const size_t imageSize = QD3D12_CompressedImageSize(tex.compressedFormat, width, height);
    tex.sysmem.resize(imageSize);

    if (pixels && imageSize)
      memcpy(tex.sysmem.data(), pixels, imageSize);
    else if (imageSize)
      memset(tex.sysmem.data(), 0, imageSize);
  }
  else
  {
...
extern "C" void glCompressedTexImage2D(GLenum, GLint mip, GLenum internalformat,
  GLsizei width, GLsizei height, GLint, GLsizei imageSize, const GLvoid* data)
{
  auto it = g_gl.textures.find(g_gl.boundTexture[g_gl.activeTextureUnit]);
  if (it == g_gl.textures.end())
    return;
  ...
  tex.compressed = true;
  tex.compressedFormat = internalformat;
  tex.dxgiFormat = MapCompressedTextureFormat(internalformat);
  ...
  const size_t expectedSize = QD3D12_CompressedImageSize(internalformat, width, height);
  const size_t copySize = (imageSize > 0) ? (size_t)imageSize : expectedSize;

  tex.sysmem.resize(copySize);
  if (data && copySize)
    memcpy(tex.sysmem.data(), data, copySize);
  else if (copySize)
    memset(tex.sysmem.data(), 0, copySize);

  EnsureTextureResource(tex);
  tex.gpuValid = false;
}

extern "C" void glCompressedTexSubImage2D(GLenum, GLint, GLint xoffset, GLint yoffset,
  GLsizei width, GLsizei height, GLenum format, GLsizei imageSize, const GLvoid* data)
{
  auto it = g_gl.textures.find(g_gl.boundTexture[g_gl.activeTextureUnit]);
  if (it == g_gl.textures.end() || !data)
    return;

  TextureResource& tex = it->second;
  if (!tex.compressed || tex.compressedFormat != format)
  {
    g_gl.lastError = GL_INVALID_OPERATION;
    return;
  }

  // S3TC updates must be block aligned unless replacing the full image.
  if ((xoffset % 4) || (yoffset % 4) ||
    ((width % 4) && (xoffset + width != tex.width)) ||
    ((height % 4) && (yoffset + height != tex.height)))
  {
    g_gl.lastError = GL_INVALID_VALUE;
    return;
  }

  const UINT blockBytes = QD3D12_BlockSizeForCompressedFormat(format);
  const int dstBlocksWide = (tex.width + 3) / 4;
  const int srcBlocksWide = (width + 3) / 4;
  const int srcBlocksHigh = (height + 3) / 4;
  const int dstBlockX = xoffset / 4;
  const int dstBlockY = yoffset / 4;

  const size_t requiredSize = QD3D12_CompressedImageSize(tex.compressedFormat, tex.width, tex.height);
  if (tex.sysmem.size() < requiredSize)
    tex.sysmem.resize(requiredSize);

  const uint8_t* src = (const uint8_t*)data;

  for (int by = 0; by < srcBlocksHigh; ++by)
  {
    uint8_t* dstRow = tex.sysmem.data() +
      (size_t)(dstBlockY + by) * (size_t)dstBlocksWide * blockBytes +
      (size_t)dstBlockX * blockBytes;

    const uint8_t* srcRow = src +
      (size_t)by * (size_t)srcBlocksWide * blockBytes;

    memcpy(dstRow, srcRow, (size_t)srcBlocksWide * blockBytes);
  }

  tex.gpuValid = false;
}

extern "C" void glTexSubImage2D(GLenum, GLint level, GLint xoffset, GLint yoffset,
  GLsizei width, GLsizei height, GLenum format, GLenum type, const GLvoid* pixels)
{
  ...
  if (tex.compressed)
  {
    glCompressedTexSubImage2D(GL_TEXTURE_2D, 0, xoffset, yoffset, width, height, format, 0, pixels);
    return;
  }
...
extern "C" void  glReadPixels(GLint, GLint, GLsizei width, GLsizei height, GLenum format, GLenum type, GLvoid* data)
{
    if (!data)
        return;
...
extern "C" void  glStencilOp(GLenum sfail, GLenum dpfail, GLenum dppass) {
...
extern "C" void  glNormalPointer(GLenum type, GLsizei stride, const void* pointer)
{
...
extern "C" void  glVertexPointer(GLint size, GLenum type, GLsizei stride, const GLvoid* ptr) {
...
extern "C" void  glColorPointer(GLint size, GLenum type, GLsizei stride, const GLvoid* ptr) {
...
extern "C" void  glTexCoordPointer(GLint size, GLenum type, GLsizei stride, const GLvoid* ptr) {
...
extern "C" void  glGeometryFlagf(GLfloat flag)
{
