...
// Force "splash image" to be loaded from a file, not a resource so same exe can be used in different localizations.
		...
		static const char* localizedPathFormat = fileName; // "Data/%s/";
		...
		if (fileImage) {
			fclose(fileImage);
			gLoadScreenBitmap = (HBITMAP)LoadImage(NULL, filePath, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
		}
		else {
			gLoadScreenBitmap = (HBITMAP)LoadImage(NULL, fileName, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
		}
#else
