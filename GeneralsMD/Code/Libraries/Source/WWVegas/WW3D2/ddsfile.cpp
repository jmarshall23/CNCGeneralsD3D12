	...
	CubeFaceSize(0)
{
    ...
    if (read_bytes != sizeof(SurfaceDesc32))
    {
    ...
    SurfaceDesc.Size = SurfaceDesc32.Size;
    SurfaceDesc.Flags = SurfaceDesc32.Flags;
    SurfaceDesc.Height = SurfaceDesc32.Height;
    SurfaceDesc.Width = SurfaceDesc32.Width;

    SurfaceDesc.Pitch = SurfaceDesc32.Pitch;
    SurfaceDesc.BackBufferCount = SurfaceDesc32.BackBufferCount;
    SurfaceDesc.MipMapCount = SurfaceDesc32.MipMapCount;

    SurfaceDesc.AlphaBitDepth = SurfaceDesc32.AlphaBitDepth;
    ...
    SurfaceDesc.CKDestBlt.ColorSpaceLowValue = SurfaceDesc32.CKDestBlt.ColorSpaceLowValue;
    SurfaceDesc.CKDestBlt.ColorSpaceHighValue = SurfaceDesc32.CKDestBlt.ColorSpaceHighValue;

    SurfaceDesc.CKSrcOverlay.ColorSpaceLowValue = SurfaceDesc32.CKSrcOverlay.ColorSpaceLowValue;
    SurfaceDesc.CKSrcOverlay.ColorSpaceHighValue = SurfaceDesc32.CKSrcOverlay.ColorSpaceHighValue;

    SurfaceDesc.CKSrcBlt.ColorSpaceLowValue = SurfaceDesc32.CKSrcBlt.ColorSpaceLowValue;
    SurfaceDesc.CKSrcBlt.ColorSpaceHighValue = SurfaceDesc32.CKSrcBlt.ColorSpaceHighValue;

    std::memcpy(&SurfaceDesc.PixelFormat, &SurfaceDesc32.PixelFormat, sizeof(SurfaceDesc.PixelFormat));
    std::memcpy(&SurfaceDesc.Caps, &SurfaceDesc32.Caps, sizeof(SurfaceDesc.Caps));

    SurfaceDesc.TextureStage = SurfaceDesc32.TextureStage;
...
#include <emmintrin.h> // SSE2
			...
			{
				const unsigned block_count = (dest_width >> 2) * (dest_height >> 2);

				if (has_hsv_shift)
				{
					// This path is harder to vectorize because Recolor() is scalar.
					// Still tighten the loop and avoid x/y address math.
					const unsigned* src_ptr = reinterpret_cast<const unsigned*>(Get_Memory_Pointer(level));
					unsigned* dest_ptr = reinterpret_cast<unsigned*>(dest_surface);

					for (unsigned i = 0; i < block_count; ++i)
					{
						const unsigned cols = *src_ptr++;
						const unsigned idx = *src_ptr++;
						...
						const unsigned new_col0 = ARGB8888_To_RGB565(col0);
						const unsigned new_col1 = ARGB8888_To_RGB565(col1);
						const unsigned new_cols = (new_col0 << 16) | new_col1;

						dest_ptr[0] = 0xFFFFFFFFu;
						dest_ptr[1] = 0xFFFFFFFFu;
						dest_ptr[2] = new_cols;
						dest_ptr[3] = idx;
						dest_ptr += 4;
					}
				...
				else
				{
					...
					const __m128i alpha_ff = _mm_set1_epi32(-1);

					for (unsigned i = 0; i < block_count; ++i)
					{
						// DXT1 block layout in src:
						// src_ptr[0] = color endpoints
						// src_ptr[1] = 2-bit indices
						...
						_mm_storeu_si128(reinterpret_cast<__m128i*>(dest_ptr), out);
			...
			else
			{
				...
				for (unsigned y = 0; y < dest_height; y += 4)
				{
					unsigned char* dest_ptr = dest_surface + y * dest_pitch;
